...
class MRNALoArmDataset(Dataset):
    """Dataset for fixed-canvas LO-ARM conditional UTR generation."""
    ...
    def __init__(
        ...
        self.target_len = self.max_utr5_tokens + 1
        ...
        for _, row in self.df.iterrows():
            ...
            target_tokens = list(utr5_tokens)
            if not self.only_utr5:
                target_tokens += [self.utr3_id] + utr3_tokens
            target_tokens += [self.eos_id]
            if len(target_tokens) > self.target_len:
                self.skipped_count += 1
                continue
    ...
    def build_prefix(self, cds_tokens):
        ...
        return prefix_ids, prefix_padding_mask
    ...
    def encode_cds_prefix(self, cds_seq):
        ...
        return self.build_prefix(cds_tokens)

    def decode_target(self, target_ids):
    ...
    def __getitem__(self, idx):
        ...
        prefix_ids, prefix_padding_mask = self.build_prefix(sample["cds_tokens"])
        return {
            "id": sample["id"],
            "cds": sample["cds"],
            "utr5": sample["utr5"],
...
def build_model_inputs(prefix_ids, prefix_padding_mask, target_canvas):
